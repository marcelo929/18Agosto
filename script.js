...
class TerminalUI {
    ...
    // NOVO MÉTODO PARA RENDERIZAR O DESAFIO DA CAIXA
...
// >>>>>>>>>>>> NOVA CLASSE DE FASE PARA O DESAFIO <<<<<<<<<<<<
class CaixaAnonimaStage extends GameStage {
    constructor(game) {
        super(game);
        this.subStage = 'grid'; // grid, finalQuestion
    }
    ...
    async verifySolution() {
        ...
        if (isCorrect) {
            ...
            this.game.ui.renderCaixaAnonima(this); // Re-renderiza para o usuário tentar de novo
        }
        this.game.isTyping = false;
    }
    ...
    async processInput(value) {
        ...
        if (answer === 'fantasma') {
            ...
            this.subStage = 'grid'; // Volta para a grelha se errar
            await this.game.ui.type(this.game.dialogue.get('caixaFail'));
            this.game.ui.renderCaixaAnonima(this);
...
window.addEventListener('DOMContentLoaded', () => {
    // Para simplificar a leitura, colei o corpo das classes inalteradas abaixo.
    // O código é o mesmo da versão anterior, apenas com as alterações/adições acima.
    
    // --- CÓPIAS DAS CLASSES INALTERADAS ---
    class AssetLoader { constructor() { this.audioContext = null; this.soundtrack = null; this.isLoaded = false; } initAudio() { if (!this.audioContext) { this.audioContext = new (window.AudioContext || window.webkitAudioContext)(); } } async loadSoundtrack(path) { if (this.isLoaded) return; this.initAudio(); const response = await fetch(path); const arrayBuffer = await response.arrayBuffer(); const audioBuffer = await this.audioContext.decodeAudioData(arrayBuffer); this.soundtrack = audioBuffer; this.isLoaded = true; } playSoundtrack() { if (!this.soundtrack || this.audioContext.state === 'suspended') { this.audioContext.resume(); } if (this.soundtrack) { const source = this.audioContext.createBufferSource(); source.buffer = this.soundtrack; source.connect(this.audioContext.destination); source.loop = true; source.start(0); } } }
