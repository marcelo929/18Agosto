...
class ChatUI {
    constructor() { /* ... (sem alterações) ... */ }
    addUserMessage(text) { /* ... (sem alterações) ... */ }
    async addBotMessage(lines) { /* ... (sem alterações) ... */ }
    renderHangman(state, dialogue) { /* ... (sem alterações) ... */ }
    renderCaixaAnonima(stage) { /* ... (sem alterações) ... */ }
    ...
    // NOVOS MÉTODOS para controlar o modo de tela cheia do desafio
    enterChallengeMode() {
...
class Dialogue { /* ... (sem alterações) ... */ }
class AssetLoader { /* ... (sem alterações) ... */ }
class GameStage { /* ... (sem alterações) ... */ }
class IntroStage extends GameStage { /* ... (sem alterações) ... */ }
class HangmanStage extends GameStage { /* ... (sem alterações) ... */ }
class FinalEnigmaStage extends GameStage { /* ... (sem alterações) ... */ }

// CLASSE ATUALIZADA para usar o modo de desafio
class CaixaAnonimaStage extends GameStage {
    constructor(game) { super(game); this.subStage = 'grid'; }

    async start() {
        // Entra no modo de tela cheia para o desafio
        this.game.ui.enterChallengeMode();
        
        // Renderiza a UI do desafio diretamente, sem mensagens de chat antes
    ...
    async processInput(value) {
        ...
        if (answer === 'fantasma') {
            ...
            // Se errar a pergunta final, volta para o modo de desafio
...
class FinalTextStage extends GameStage { /* ... (sem alterações) ... */ }
class Game { /* ... (sem alterações) ... */ }


// --- CÓDIGO COMPLETO DAS CLASSES (JÁ ADAPTADO) ---
// (Copie tudo abaixo para seu script.js)

class ChatUI { constructor() { this.messageContainer = document.querySelector('.message-container'); this.interactiveArea = document.getElementById('interactive-area'); this.userInput = document.getElementById('userInput'); this.inputLine = document.querySelector('.input-line'); this.finalScene = document.getElementById('finalScene'); this.finalText = document.getElementById('finalText'); } addUserMessage(text) { const messageDiv = document.createElement('div'); messageDiv.className = 'message user-message'; messageDiv.textContent = text; this.messageContainer.appendChild(messageDiv); this.scrollToBottom(); } async addBotMessage(lines) { this.disableInput(); if (typeof lines === 'string') lines = [lines]; const typingDiv = document.createElement('div'); typingDiv.className = 'message bot-message'; typingDiv.innerHTML = '<div class="typing-indicator"><span></span><span></span><span></span></div>'; this.messageContainer.appendChild(typingDiv); this.scrollToBottom(); await new Promise(r => setTimeout(r, 1200)); this.messageContainer.removeChild(typingDiv); for (const line of lines) { const messageDiv = document.createElement('div'); messageDiv.className = 'message bot-message'; messageDiv.textContent = line; this.messageContainer.appendChild(messageDiv); this.scrollToBottom(); await new Promise(r => setTimeout(r, 400)); } this.enableInput(); } renderHangman(state, dialogue) { this.clearInteractiveArea(); const currentWordData = dialogue.getHangmanWord(state.currentWordIndex); const revealedWord = currentWordData.word.split('').map(letter => (state.normalizedGuessedLetters.includes(this.normalizeString(letter))) ? letter : '＿').join(' '); let hangmanText = `Palavra: ${revealedWord}\n`; hangmanText += `Dica: "${currentWordData.hint}"\n`; hangmanText += `Letras: [${state.guessedLetters.join(', ')}] | Erros: ${state.wordErrors}/${state.maxErrors}`; const messageDiv = document.createElement('div'); messageDiv.className = 'bot-message'; messageDiv.style.whiteSpace = 'pre-wrap'; messageDiv.textContent = hangmanText; this.interactiveArea.appendChild(messageDiv); this.scrollToBottom(); } renderCaixaAnonima(stage) { this.clearInteractiveArea(); this.disableInput(); const dialogue = stage.game.dialogue; const hintsHTML = dialogue.getCaixaHints().map(hint => `<p>${hint}</p>`).join(''); this.interactiveArea.innerHTML = `<div class="bot-message" style="max-width: 100%;"><p><strong>Codenomes:</strong> ${dialogue.getCaixaCodenames().join(', ')}.</p><p><strong>Arsenal:</strong> ${dialogue.getCaixaArsenal().join(', ')}.</p><br><p><strong>Fragmentos do Servidor:</strong></p>${hintsHTML}</div>`; const codenames = dialogue.getCaixaCodenames(); const arsenal = dialogue.getCaixaArsenal(); const defaultOption = `<option value="">Selecione...</option>`; const codenameOptions = defaultOption + codenames.map(c => `<option value="${c}">${c}</option>`).join(''); const arsenalOptions = defaultOption + arsenal.map(a => `<option value="${a}">${a}</option>`).join(''); let gridHTML = `<table id="challenge-grid"><thead><tr><th>Posição</th><th>Codinome</th><th>Arsenal</th></tr></thead><tbody>`; for (let i = 1; i <= 5; i++) { gridHTML += `<tr><td>${i}</td><td><select id="codename-${i}">${codenameOptions}</select></td><td><select id="weapon-${i}">${arsenalOptions}</select></td></tr>`; } gridHTML += `</tbody></table><button id="verifyBtn">Verificar Solução</button>`; this.interactiveArea.innerHTML += gridHTML; document.getElementById('verifyBtn').addEventListener('click', () => stage.verifySolution()); } showFinalScene(text) { document.getElementById('terminal').classList.add('hidden'); this.finalScene.classList.remove('hidden'); this.finalText.textContent = text; } normalizeString(str) { return str.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); } clearInteractiveArea() { this.interactiveArea.innerHTML = ''; } enableInput() { this.userInput.disabled = false; this.userInput.focus(); } disableInput() { this.userInput.disabled = true; } getValueAndClear() { const value = this.userInput.value; this.userInput.value = ''; return value; } scrollToBottom() { this.messageContainer.scrollTop = this.messageContainer.scrollHeight; } }
class Dialogue { constructor() { this.content = gameContent; } get(key) { return this.content.script[key]; } getRandomLine(key) { const lines = this.get(key); return lines[Math.floor(Math.random() * lines.length)]; } getHangmanWords() { return this.content.hangmanWords; } getHangmanWord(index) { return this.content.hangmanWords[index]; } getCaixaHints() { return this.content.caixaHints; } getCaixaCodenames() { return this.content.caixaCodenames; } getCaixaArsenal() { return this.content.caixaArsenal; } getCaixaSolution() { return this.content.caixaSolution; } }
...
class Game { constructor() { this.ui = new ChatUI(); this.dialogue = new Dialogue(); this.assetLoader = new AssetLoader(); this.currentStage = null; } async start() { this.setupEventListeners(); this.assetLoader.loadSoundtrack('./audio/soundtrack.mp3'); this.transitionTo(new IntroStage(this)); } transitionTo(newStage) { this.currentStage = newStage; if (this.currentStage) { this.currentStage.start(); } } async processInput() { if (!this.currentStage) return; const value = this.ui.getValueAndClear().trim(); if (value) { this.ui.addUserMessage(value); await this.currentStage.processInput(value); } } setupEventListeners() { this.ui.inputLine.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); const value = this.ui.getValueAndClear().trim(); this.processInput(value); } }); document.body.addEventListener('click', () => this.assetLoader.initAudio(), { once: true }); } }
